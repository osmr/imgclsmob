...
def oth_mobilenet_v3_large(**kwargs):
...
def oth_mobilenet_v3_small(**kwargs):
    r"""MobileNetV3 model from the
    `"Searching for MobileNetV3"
    <https://arxiv.org/abs/1905.02244>`_ paper.
    Parameters
...
def _test():
    ...
    models = [
        oth_mobilenet_v3_small,
        oth_mobilenet_v3_large,
    ]
    ...
    for model in models:
        ...
        x = mx.nd.random.randn(12, 3, 224, 224, ctx=ctx)
        ...
        assert (model != oth_mobilenet_v3_small or weight_count == 2945288)
        assert (model != oth_mobilenet_v3_large or weight_count == 5481752)
