...
class NTSNet(nn.Module):
    ...
    def forward(self, x):
        ...
        x_f = self.pretrained_model_avgpool(rpn_feature)
        x_f = x_f.view(x_f.size(0), -1)
        feature = nn.Dropout(p=0.5)(x_f)
        ...
        x_f2 = part_imgs.detach()
        x_f2 = self.pretrained_model(x_f2)
        x_f2 = self.pretrained_model_avgpool(x_f2)
        x_f2 = x_f2.view(x_f2.size(0), -1)
        part_features = nn.Dropout(p=0.5)(x_f2)
