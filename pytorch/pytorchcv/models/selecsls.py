...
class SelecSLSHeadUnit(nn.Module):
    """
    SelecSLS head unit.
    ...
    out_channels : int
        Number of output channels.
    use_kernel3 : bool
        Whether to use 3x3 (instead of 1x1) kernel.
    stride : int or tuple/list of 2 int
        Strides of the branch convolution layers.
    """
    def __init__(self,
                 ...
                 use_kernel3,
                 stride):
        super(SelecSLSHeadUnit, self).__init__()
        conv_block_class = conv3x3_block if use_kernel3 else conv1x1_block

        self.conv = conv_block_class(
            in_channels=in_channels,
            out_channels=out_channels,
            stride=stride)
        ...
        for i, channels_per_stage in enumerate(channels):
            ...
            for j, out_channels in enumerate(channels_per_stage):
                ...
                if k < 0:
                    ...
                    unit = SelecSLSHeadUnit(
                        in_channels=in_channels,
                        out_channels=out_channels,
                        use_kernel3=(kernels3[k][j] == 1),
                        stride=stride)
