...
from timm.models.layers import SpaceToDepthModule, AntiAliasDownsampleLayer, InplaceAbn, ClassifierHead, SEModule

__all__ = ['oth_tresnet_m', 'oth_tresnet_l', 'oth_tresnet_xl']
...
class TResNet(nn.Module):
    def __init__(self, layers, in_chans=3, num_classes=1000, width_factor=1.0, no_aa_jit=False,
                 global_pool='fast', drop_rate=0., in_channels=3, in_size=(224, 224)):
...
def oth_tresnet_xl(pretrained=False, **kwargs):
...
def _test():
    ...
    for model, size in models:
        ...
        assert (model != oth_tresnet_m or weight_count == 31389032)
        assert (model != oth_tresnet_l or weight_count == 55989256)
        assert (model != oth_tresnet_xl or weight_count == 78436244)

        assert (model != tresnet_m_448 or weight_count == 31389032)
        assert (model != tresnet_l_448 or weight_count == 55989256)
        assert (model != tresnet_xl_448 or weight_count == 78436244)
