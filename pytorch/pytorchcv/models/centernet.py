...
class CenterNetHeatmapMaxDet(nn.Module):
    ...
    def forward(self, x):
        ...
        # wh = x[:, -4:-2]
        # reg = x[:, -2:]
        # _, _, out_h, out_w = heatmap.shape
        # scores, indices = heatmap.view((heatmap.size(0), -1)).topk(k=self.topk)
        # topk_classes = (indices / (out_h * out_w)).type(torch.float32)
        # topk_indices = indices.fmod(out_h * out_w)
        # topk_ys = topk_indices / out_w
        # topk_xs = topk_indices.fmod(out_w)
        # center = reg.permute(0, 2, 3, 1).view((reg.size(0), -1, 2))
        # wh = wh.permute(0, 2, 3, 1).view((wh.size(0), -1, 2))
        # batch_indices = F.cast(torch.arange(256).slice_like(
        ...
        # bboxes = torch.cat([tmp.expand_dims(-1) for tmp in bboxes], dim=-1)
        #
        # bboxes = bboxes * self.scale
        # topk_classes = topk_classes.expand_dims(axis=2)
        # scores = scores.expand_dims(axis=2)
        # result = torch.cat((bboxes, topk_classes, scores), dim=2)
        # return result
        return x
